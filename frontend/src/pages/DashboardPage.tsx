...
const getIcon = (type: string) => {
  switch(type) {
    case 'img': return <Image size={20} className="text-gray-500" />;
    case 'pdf': return <FileText size={20} className="text-gray-500" />;
    case 'video': return <Video size={20} className="text-gray-500" />;
    case 'text': return <File size={20} className="text-gray-500" />;
    case 'audio': return <Music size={20} className="text-gray-500" />;
    case 'xlsx': return <Table size={20} className="text-gray-500" />;
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <div className="p-2 bg-blue-50 rounded-xl text-blue-500"><Folder size={20} /></div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <div className="p-2 bg-indigo-50 rounded-xl text-indigo-500"><Users size={20} /></div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-50 flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2">
            <div className="p-2 bg-yellow-50 rounded-xl text-yellow-500"><Star size={20} /></div>
      ...
      {!hasAnyFiles ? (
         <div className="py-12 text-center text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-2xl bg-white">
            ...
            <p>No files uploaded yet. <span className="text-blue-500 cursor-pointer">Upload a file</span> to get started.</p>
         </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-50">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link to={`/directories/${personalStorageId}`} className="text-sm text-blue-500 hover:text-blue-600 font-medium">View all</Link>
            </div>
      ...
      {hasAnyFiles && (
        <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-gray-50">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link to="/favorites" className="text-sm text-blue-500 hover:text-blue-600 font-medium">View all</Link>
          </div>
          
          {favorites.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
               {favorites.slice(0, 3).map((fav) => (
                  <Link to={`/files/${fav.id}`} key={fav.id} className="flex items-center gap-3 bg-gray-50 p-3 rounded-xl hover:bg-gray-100 transition-colors">
                    <div className="p-2 bg-white rounded-lg shadow-sm shrink-0">{getIcon(fav.type)}</div>
                    <div>
                      <p className="text-sm font-medium text-gray-700 truncate max-w-[120px]">{fav.name}</p>
            ...
            <div className="py-8 text-center text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-xl">
              ...
              <p>No favorites yet. <span className="text-blue-500 cursor-pointer">Add a file</span> for quick access.</p>
            </div>
          )}
        </div>
      )}
